...
Options:
   ...
   -p, --snapshot <snapshot-name>       Use to specify the snapshot name
   -o, --option <delete|show>           Use to manipulate actions on the snapshot
   -f, --force                          Use to ignore aggr. threshold OR ignore snapshot interactive delete
Examples:
Usage like: $0 -n Netapp01 -v vol1 -s +50g
OR
Usage like: $0 -n Netapp01 -v vol1 -s
OR
Usage like: $0 -n Netapp01 -v vol1 -p hourly.0 -o delete [-f]
EOF
quit 1
}
...
then
	...
	SNAP_RESERVE=$(\rsh $NetApp snap reserve $VOL | awk '{sub(/%/,""); print $7}')
        if [ ! -z $SNAPSHOT ];then
		SNAPS=(`rsh $NetApp snap list -b $VOL | awk '{print $1}' |tail -n +7`)
	fi
...
then
	[ ! -z $SNAPSHOT ] && echo -e "\n\e[31mERROR: Can't use both resize and snapshot options\e[0m" && quit 300
	environment_check
...
then
	...
	echo -e "\n\e[032mDONE\e[0m"
	quit 0
elif [ ! -z $NetApp ] && [ ! -z $VOL ] && [ -z $check_s ] && [ -z $RESIZE ] && [ ! -z $SNAPSHOT ]
then
	environment_check
